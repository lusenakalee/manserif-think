...
const OWNER_EMAIL = process.env.CONTACT_TO;

export async function POST(req: NextRequest) {
  try {
    // Make sure required email environment variables exist
    if (!FROM_EMAIL || !OWNER_EMAIL) {
      console.error(
        "[quote-request] Missing MAIL_FROM or CONTACT_TO environment variable"
      );
  ...
  return NextResponse.json(
      ...
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#111;">
          <h2 style="margin-bottom:4px;">
            Thanks for your request, ${userName.split(" ")[0]}!
          </h2>

          <p style="color:#666;margin-top:0;">
            We've received your enquiry and will get back to you with pricing shortly.
          </p>
          ...
          <table style="width:100%;border-collapse:collapse;">
            ...
            <tr>
              ...
              <td style="padding:6px 0;font-size:12px;color:#999;">
                ${productId}
              </td>
            </tr>
          </table>
    ...
    if (customerEmail.error) {
      console.error(
        "[quote-request] Customer email failed:",
        customerEmail.error
      );

      return NextResponse.json(
        { error: "Request received, but confirmation email failed" },
        { status: 500 }
      );
    }
