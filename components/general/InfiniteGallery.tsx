...
type ImageItem =
	...
	| {
			src: string;
			alt?: string;
			/** Optional caption rendered under the image (e.g. exhibit title) */
			title?: string;
			/** Optional URL the image should link to when clicked */
			href?: string;
	  };
...
interface PlaneData {
	...
	opacity: number; // Current fade opacity, mirrored to the HTML caption
}
	...
	return (
		<group position={position}>
			...
			{title ? (
				...
				>
					<p className="text-white text-center font-serif text-sm md:text-base tracking-tight">
						{title}
					</p>
				</Html>
			) : null}
...
function GalleryScene({
	...
	// Load textures manually so a single broken/unavailable Sanity image
	// does not throw a React/Suspense error and crash the whole gallery.
	...
	useEffect(() => {
		...
		// Sanity CDN images are cross-origin resources.
		loader.setCrossOrigin('anonymous');

		const loaded: (THREE.Texture | null)[] = new Array(
		...
		normalizedImages.forEach((image, index) => {
			...
			loader.load(
				...
				(texture) => {
					...
					// Update progressively as each image finishes loading.
					setTextures([...loaded]);
				},
				undefined,
				(error) => {
					// A bad image should not crash the entire Three.js scene.
					console.warn(
						`InfiniteGallery: failed to load image: ${image.src}`,
						error
					);
					if (!cancelled) {
						loaded[index] = null;
	...
	return (
		<>
			...
			const aspect = texture.image && typeof texture.image === 'object' && 'width' in texture.image
