...
const Section: React.FC<{
  ...
  description: string;
  slug: string;
}> = ({ video, image, imageAlt, tag, title, description, slug }) => {
  ...
  return (
    ...
    >
      {/* Parallax background — video if available, otherwise hero image */}
      <motion.div
      ...
      <div className="flex flex-col justify-end h-full pb-20 px-6 sm:px-12 md:px-20 lg:px-24">
        <div className="flex flex-col gap-3 max-w-2xl">
          ...
          <Link
            href={`/exhibits/${slug}`}
            className="mt-2 inline-flex w-fit items-center gap-2 border border-white/40 bg-black/20 backdrop-blur-sm px-5 py-2.5 text-[11px] uppercase tracking-[0.2em] transition-all duration-200 hover:bg-white hover:text-black"
          >
      ...
      {/* Controls — bottom left, only shown when there's a video to control */}
      {hasVideo && (
